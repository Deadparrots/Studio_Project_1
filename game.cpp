...
void moveCharacter()
{
	...
	if (Weapons[currentWeapon].Clip > 0) // Checks if the weapon has enough ammo to fire
	{
...
void renderUI()
{
	COORD UIBG;
	COORD UI;
	for (int i = 0; g_Console.getConsoleSize().X > i; i++) // Leaves the first 3 bars for the UI
	{
		UIBG.X = i;
		for (int j = 0; 3 > j; j++) // Sets UI Height to 3
		{
			UIBG.Y = j;
			g_Console.writeToBuffer(UIBG," ",0x00); // Black Overlay
		}
	}
	...
	string display = to_string(Lives);
