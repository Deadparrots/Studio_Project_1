...
void getInput(void)
{
	...
	g_abKeyPressed[K_Q] = isKeyPressed(81); // For SHOP
}
...
void moveCharacter()
{
	...
	//	{
	//	case 0:
	//		if (g_shopkeeper.m_bActive == true && Map[g_shopkeeper.m_cLocation.X + g_shopkeeper.m_cLocation.Y * 80 + 1] == ' ')
	//			g_shopkeeper.m_cLocation.X++;
	//		break;
	//	case 1:
	//		if (g_shopkeeper.m_bActive == true && Map[g_shopkeeper.m_cLocation.X + g_shopkeeper.m_cLocation.Y * 80 + 80] == ' ')
	//			g_shopkeeper.m_cLocation.Y++;
	//		break;
	//	case 2:
	//		if (g_shopkeeper.m_bActive == true && Map[g_shopkeeper.m_cLocation.X + g_shopkeeper.m_cLocation.Y * 80 - 1] == ' ')
	//			g_shopkeeper.m_cLocation.X--;
...
void shop() {
	...
	case MMLive:
		m.Y = 17;
		m.X = m.X / 2 - 21;
		g_Console.writeToBuffer(m, "25 Coin: 1 live", 0x0E);
		...
		if (!Weapon1)
		{
			...
			g_Console.writeToBuffer(m, "50 Coin: Crossbow", 0x03);
		}
		else if (Weapon1)
		{
			m.Y += 2;
			m.X = g_Console.getConsoleSize().X / 2 - 21;
			g_Console.writeToBuffer(m, "50 Coin: Crossbow", 0x0D);
		}
		...
		if (!Weapon2)
		{
			m.Y = 17;
			m.X = m.X / 2 + 32;
			g_Console.writeToBuffer(m, "70 Coin: Rifle", 0x03);
		}
		else if (Weapon2)
		{
			m.Y = 17;
			m.X = m.X / 2 + 32;
			g_Console.writeToBuffer(m, "70 Coin: Rifle", 0x0D);
		}
	...
	case MMW1:
		m.Y = 17;
		m.X = m.X / 2 - 21;
		g_Console.writeToBuffer(m, "25 Coin: 1 live", 0x03);

		m.Y += 2;
		m.X = g_Console.getConsoleSize().X / 2 - 21;
		g_Console.writeToBuffer(m, "50 Coin: Crossbow", 0x0E);
	...
	case MMW2:
		...
		m.Y = 17;
		m.X = m.X / 2 + 32;
		g_Console.writeToBuffer(m, "70 Coin: Rifle", 0x0E);
...
void shopoption() 
{
	...
	if (g_abKeyPressed[K_SPACE] && MMSelect == MMW1 && g_eGameState == S_SHOP && !Weapon1) // CONTINUE_GAME
	{
		if (Coin >= 50 && !Weapon1)
		{
			Coin -= 50;
	...
	if (g_abKeyPressed[K_SPACE] && MMSelect == MMW2 && g_eGameState == S_SHOP && !Weapon2) // CONTINUE_GAME
	{
		if (Coin >= 70 && !Weapon2)
		{
			Coin -= 70;
	...
	if (g_abKeyPressed[K_SPACE] && MMSelect == MMBack && g_eGameState == S_SHOP) // QUIT_GAME
	{
		if (StageType == EBoss)
		{
			boss_battle_init();
...
void convertToString()
{
	...
	myfile11.close();
	std::fstream myfile12("map/Shop.txt");
	for (short i = 0; i < 25 * 80; i++)
	{
		myfile12.seekg(i);
		char * buffer = new char[1];
